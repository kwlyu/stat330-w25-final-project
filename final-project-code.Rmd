...
```{r, cache=TRUE}
control_clmm_full <- clmm(limit ~ 1 + age_std + gender + race + edu + city + caff_std + nsigns_ssb_std + num_kids + days_since_ban_std + kcal_std + fv_std + (1 | location) + (1 | round),
                          control = list(method = "nlminb",
                                         useMatrix = T,
                                         maxIter = 200,
                                         gradTol = 1e-4,
...
control_clmm_red <- clmm(limit ~ 1 + age + gender + edu + num_kids + (1 | location) + (1 | round), control = list(trace = 1),
                     data = reduced_data, link = "logit")
...
conf_mat(control_results_log, truth = limit, estimate = fit)
