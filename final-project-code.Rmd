...
control_clmm_red <- clmm(limit ~ 1 + age + gender + edu + caff + nsigns_ssb + num_kids + (1 | location) + (1 | round), control = list(trace = 1),
                     data = reduced_data, link = "logit")
...
# Caffeine
ggplot(data = na.omit(reduced_data) %>% filter(caff > 0), aes(x = log(caff + 1) , y = limit)) + 
  geom_boxplot() +
  labs(x = "Caffeine", y = "Limit")

# Survey round
ggplot(data = na.omit(reduced_data), aes(x = round, fill = limit)) +
  geom_bar(position = "fill") + 
  scale_y_continuous(labels = scales::percent) + 
...
# Advertisement count
ggplot(data = na.omit(reduced_data), aes(y = nsigns_ssb, x = limit)) +
  geom_boxplot() + 
...
```{r}
# Crossed Effect?
stores_r1 <- unique(reduced_data[which(reduced_data$round == 1),]$location)
stores_r2 <- unique(reduced_data[which(reduced_data$round == 2),]$location)
stores_r3 <- unique(reduced_data[which(reduced_data$round == 3),]$location)

length(intersect(stores_r1, stores_r2))
length(intersect(stores_r1, stores_r3))
length(intersect(stores_r3, stores_r2))
```
